...
public class ResultReader {
   ...
   private Test readTest(String testLine, boolean isTotals, boolean hasTPS) {
      ...
      if (scanner.hasNextDouble()) {
         ...
         String fbmt = scanner.next();
         double firstByteMeanTime = -1.0;
         if(fbmt.endsWith(")")) {
             firstByteMeanTime = Double.valueOf(fbmt.substring(0, fbmt.length() - 1)) ;
         } else {
             firstByteMeanTime = Double.valueOf(fbmt);
          }
          
         String name = isTotals ? "" : scanner.next().replaceAll("\"", "");
         System.out.println(id + ' ' +testCount+' '+errorCount+' '+meanTestTime+' '+testStdDevTime
                               +' '+tps+' '+meanRespLength+' '+respBytesPrSecond+' '+respErrorCount
                               +' '+resolveHostMeanTime+' '+establishConnMeanTime+' '+firstByteMeanTime+' '+name);
