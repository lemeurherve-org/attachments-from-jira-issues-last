	...
	public boolean checkout(AbstractBuild build, Launcher launcher,
		...
		for(URLTuple tuple : urls) {
            ...
            String userinfo = url.getUserInfo();
				//listener.getLogger().append("userinfo: " + userinfo + "\n");

            if(userinfo!=null) {
               String[] userpass = userinfo.split(":");
               if(userpass.length>0) {
                  String username = userpass[0];
      				//listener.getLogger().append("username: " + username + "\n");
                  String password = "";
                  if(userpass.length>1) {
                     password = userpass[1];
         				//listener.getLogger().append("password: " + password + "\n");
                  }
                  String auth = "Basic " + (new sun.misc.BASE64Encoder().encode((username + ":" + password).getBytes()));
                  conn.setRequestProperty("Authorization", auth);
                  //listener.getLogger().append("authorization set\n");
               }
            }
