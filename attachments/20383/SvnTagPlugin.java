        ...
        for (SubversionSCM.ModuleLocation ml : moduleLocations) {
			...
			try{
				evaledTagBaseURLStr =
					...
					URLEncoder.encode(evalGroovyExpression(env, tagBaseURLStr,
                            locationPathElements), "UTF8");
				} catch (UnsupportedEncodingException e) {
					logger.println("Failed to encode SVN repo URL. " +
			...
			try{
				remoteURLStr =
					URLEncoder.encode(ml.remote, "UTF8");
				} catch (UnsupportedEncodingException e) {
					logger.println("Failed to encode SVN repo URL. " +
                        e.getLocalizedMessage());
					return false;
            ...
            try {
                parsedTagBaseURL = SVNURL.parseURIEncoded(
						...
						URLDecoder.decode(repoURI.resolve(evaledTagBaseURLStr).toString(), "UTF8"));
                logger.println(
                ...
                logger.println(
					...
					logger.println("Failed to decode SVN repo URL. " +
                        e.getLocalizedMessage());
					return false;
			}
            ...
            try {
                ...
                if (null != errorMsg) {
                    logger.println("Failed tag " + errorMsg.getFullMessage());
                    return false;
                } else {
                    logger.println("Tagged Info " +Messages.Tagged(commitInfo.getNewRevision()));
                }
            } catch (SVNException e) {
                logger.println("Failed exception"  + Messages.CopyFailed(e.getLocalizedMessage()));
                return false;
            }
