...
public final class EmmaBuildAction extends CoverageObject<EmmaBuildAction> implements HealthReportingAction, StaplerProxy {
    ...
    public static EmmaBuildAction load(AbstractBuild<?,?> owner, Rule rule, EmmaHealthReportThresholds thresholds, FilePath... files) throws IOException {
        ...
        return new EmmaBuildAction(owner,rule,ratios.get("class"),ratios.get("method"),ratios.get("block"),ratios.get("line"),thresholds);
    }
    ...
    private static Map<String, Ratio> loadRatios(InputStream in, Map<String, Ratio> r) throws IOException, XmlPullParserException {
        ...
        for( int i=0; i<4; i++ ) {
            ...
            if (!r.containsKey(type)) {
                r.put(type, Ratio.parseValue(v));
            } else {
                r.get(type).addValue(v);
            }
