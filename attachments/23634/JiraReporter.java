...
public class JiraReporter extends Notifier {
    ...
    public boolean perform(final AbstractBuild build,
        ...
        debugLog(listener,
                 String.format("%s Workspace is %s%n", pInfo, build.getWorkspace().toString())
                );
...
//      }
    ...
    private void printResultItems(final List<CaseResult> failedTests,
        ...
        for (CaseResult result : failedTests) {
            ...
            String affectedFile = result.getErrorStackTrace();
...
//          if (result.getAge() > 0) {
                ...
                try {
                    ...
                    // The Google C++ Test Framework inserts control characters in the test output (like carriage returns)
                    // so we need to remove them
                    // Also, when the test name is provided, it have the full path, so we have to escape the \ character in the path
                    //
                    String testName = CharMatcher.JAVA_ISO_CONTROL.replaceFrom(result.getName(),' ');  // replace any control characters with spaces
                    String testClassName = CharMatcher.JAVA_ISO_CONTROL.replaceFrom(result.getClassName(),' '); // replace any control characters with spaces
                    String testErrorDetails = CharMatcher.JAVA_ISO_CONTROL.replaceFrom(result.getErrorDetails(),' '); // replace any control characters with spaces
                    String testErrorStackTrace  = CharMatcher.anyOf("\\").replaceFrom(CharMatcher.JAVA_ISO_CONTROL.replaceFrom(result.getErrorStackTrace(),' '),"\\\\"); // replace any control characters with spaces and escape any backslashes in paths
                    
                    String jsonPayLoad = new String("{\"fields\": {\"project\": {\"key\": \"" + this.projectKey + "\"},\"summary\": \"The test " + testName + " failed " + testClassName + ": " + testErrorDetails + "\",\"description\": \"Test class: " + testClassName + " -- " + testErrorStackTrace + "\",\"issuetype\": {\"name\": \"Bug\"}}}");

                    debugLog(listener,
                         String.format("JSON payload: %s",
                            jsonPayLoad)
                        );
