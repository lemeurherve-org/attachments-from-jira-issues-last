...
public class MultiJobBuild extends Build<MultiJobProject, MultiJobBuild> {
	...
	public List<SubBuild> getBuilders() {
	    MultiJobBuild multiJobBuild = getParent().getNearestOldBuild(getNumber()); // CHANGED really?  Why not use self here?
	...
	public void addSubBuild(String parentJobName, int parentBuildNumber,
		...
		// Try to fix the issue in the "perform" method of the builder class.  It will try and
		// add the latest build of a subbuild no matter who generated it!  We need to find the
		// actual parent build number of the subbuild being requested here and verify it matches the 
		// parent build number specified in the argument list.
		Run subRun = getBuild(subBuild);
		Cause.UpstreamCause cause = (Cause.UpstreamCause) subRun.getCause(Cause.UpstreamCause.class);
		int parentRunBuildNum = -1; // The actual parent build number of this subBuild

		if (cause != null) {
		    // only add subbuilds that have a upstream cause
		    //		    parentRun = cause.getUpstreamRun();
		    parentRunBuildNum = cause.getUpstreamBuild();

		    if (parentBuildNumber == parentRunBuildNum) {
			// Remove duplicates of this request.  Builder object will keep trying to add it until the job completes.
			for (SubBuild subbuild : getSubBuilds()) {
			    if (subbuild.getJobName().equals(jobName) && subbuild.getBuildNumber() == buildNumber) {
